...
export default function DateTimeInput({ value, onChange, label }: DateTimeInputProps) {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full space-y-1">
      ...
      {open && (
        <div className="bg-base-100 border-base-300 absolute top-full left-0 z-10 mt-2 rounded-lg border p-2 shadow-lg">
          <DayPicker mode="single" selected={value ?? undefined} onSelect={handleDateSelect} />
        </div>
      )}
    </div>
  );
